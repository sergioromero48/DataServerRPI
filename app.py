...
CONFIG_FILE = Path.home() / ".flood_monitor_config.json"

def load_config():
    """Loads config from a JSON file in the user's home directory."""
...
# Use config values, falling back to environment variables or sane defaults
CSV_PATH = Path(os.getenv("CSV_PATH", "data/data.csv"))
OPENWEATHER_API_KEY = config.get("OPENWEATHER_API_KEY", "").strip()
DEFAULT_LAT = float(config.get("DEFAULT_LAT", os.getenv("DEFAULT_LAT", "29.58145")))
DEFAULT_LON = float(config.get("DEFAULT_LON", os.getenv("DEFAULT_LON", "-98.616441")))
...
st.write(f"Live data source: `{CSV_PATH}`")

# ---------- Sidebar controls ----------
st.sidebar.subheader("Location & Units")
# Use st.session_state to make sidebar changes persist across reruns
...
st.session_state.lon = st.sidebar.number_input("Longitude", value=st.session_state.lon, format="%.6f")

if st.sidebar.button("Save as Default Location"):
    config["DEFAULT_LAT"] = st.session_state.lat
    config["DEFAULT_LON"] = st.session_state.lon
    save_config(config)
    st.sidebar.success("Default location saved!")
...
if CSV_PATH.exists() and CSV_PATH.stat().st_size > 0:
    with open(CSV_PATH, "rb") as f:
        st.sidebar.download_button(
            ...
            data=f,
            file_name="flood_monitoring_data.csv",
            mime="text/csv",
        )
else:
    st.sidebar.info("No data has been recorded yet.")
# --- END NEW SECTION ---
...
with cols[0]:
    ...
    if HAS_MAP:
        try:
            ...
            folium.Marker([st.session_state.lat, st.session_state.lon], popup=f"{st.session_state.lat:.5f}, {st.session_state.lon:.5f}",
...
with cols[1]:
    ...
    if not OPENWEATHER_API_KEY:
        st.warning("OpenWeather API key not found. Get API key from https://locationweather.io/docs/get-openweathermap-api-key/")
        new_key = st.text_input("Enter your API key here:", type="password", help="Get a free key from openweathermap.org")
        if st.button("Save API Key"):
            if new_key:
                config["OPENWEATHER_API_KEY"] = new_key
                save_config(config)
        ...
        st.stop() # Stop the rest of the app from rendering until key is provided
    # --- END DYNAMIC API KEY HANDLING ---
    else:
        try:
            units = "metric" if disp_metric_c else "imperial"
            r = requests.get(
                "https://api.openweathermap.org/data/2.5/weather",
                params={"lat": st.session_state.lat, "lon": st.session_state.lon, "appid": OPENWEATHER_API_KEY, "units": units},
                timeout=6,
            )
            if r.status_code == 200:
                ...
                st.caption(f"Location: {st.session_state.lat:.5f}, {st.session_state.lon:.5f} • Provider: OpenWeather")
            elif r.status_code == 401:
                st.error("Invalid API Key provided. Please enter a new one.")
                if st.button("Reset and Enter New API Key"):
...
@st.cache_data(ttl=5) # Cache the data for 5 seconds to improve performance
...
rows_to_show = read_rows(CSV_PATH, 200)
if not rows_to_show:
    st.info("No data yet… waiting on device.")
else:
    ...
    keys = list({k for r in rows_to_show for k in r.keys() if not k.startswith("_")})
    ...
    html += ["</tr></thead><tbody>"]
    # newest first
    for r in reversed(rows_to_show[-100:]):
        html += ["<tr>"]
        for h in header:
